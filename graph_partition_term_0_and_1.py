...
def calculate_gain_A_o(idx,i, A_o, B_o):
    ...
    pos=list(set(in_nids).intersection(set(B_src)))
    neg=list(set(in_nids).intersection(set(A_src)))
    ...
    if gain >= 0:
        ...
        if gain==0:
            print((i in B_o))
            print(in_nids)
            print('\t\t'+str(gain_pos)+' '+str(pos))
            print('\t\t\t'+str(gain_neg)+' '+str(neg))
    if gain > maxDegree:
        maxDegree = max(gain, maxDegree)
        print(j)
        print(gain)
        
    gains=initializeBucketSort(i, gain, 0)
    return  (idx,gain)  
...
def calculate_gain_B_o(idx,i, A_o, B_o):
    ...
    if gain > maxDegree:
        ...
        print(j)
        print(gain)
    gains=initializeBucketSort(i, gain, 1)
    return  (idx,gain)  
...
def initializeGain():
    ...
    tt=time.time()
    gains={}
    # A_o,B_o=get_two_partition_seeds() # global bit_dict
    A_o=[k for k in bit_dict if bit_dict[k] == 0]
    B_o=[k for k in bit_dict if bit_dict[k] == 1]
    print('A_o,B_o time:  ', time.time()-tt)
    ...
    for idx, i in enumerate(A_o):
        ...
        #     print('\t\t'+str(gain_pos)+'\t'+str(gain_neg))
        if idx>0 and idx%100==0:
            print('\t : ', num_zero_p/idx)    
        gains=initializeBucketSort(i,gain, 0) # 0: left side bucket
    print('rate of zero positive: ', num_zero_p/idx)    
        # calculate_gain_A_o(idx,i, A_o, B_o)
    ...
    for idx, i in enumerate(B_o):
        ...
        #     print('\t\t'+str(gain_pos)+'\t'+str(gain_neg))
        if idx>0 and idx%100==0:
            print('\t : ', num_zero_n/idx)    
        gains=initializeBucketSort(i, gain, 1) # 1: right side bucket
    print('rate of zero : ', num_zero_n/idx)   
        # calculate_gain_B_o(idx,i, A_o, B_o)
...
def updateGain(locked_nodes, cur_rate, alpha,ideal_part_in_size):
    ...
    if side==0:
        ...
        while not ifFound:
            ...
            while bucketIndex:
                ...
                if   not locked_nodes[i] and move_nid_balance_redundancy_check(bit_dict, side, i,alpha, cur_rate,ideal_part_in_size):
                # if 
                # not locked_nodes[i] and move_nid_balance_check(bit_dict, side, i, alpha):
                    maxGainNid = i
                    ifFound = True
        ...
        while not ifFound:
            ...
            while bucketIndex:
                ...
                if   not locked_nodes[nid] and move_nid_balance_redundancy_check(bit_dict, side, nid,alpha, cur_rate,ideal_part_in_size):
...
# def updateRed(locked_nodes, cur_rate, alpha,ideal_part_in_size):
#     global maxGainIndex, maxLeftGainIndex, maxRightGainIndex, maxDegree, leftBuckets, rightBuckets 
#     global side, bit_dict
...
#         gain=gain_pos-gain_neg 
#         if gain>0:
#             print('A_o gain is positive '+str(gain))
#             if not locked_nodes[i] and move_nid_balance_redundancy_check(bit_dict, side, i,alpha, cur_rate,ideal_part_in_size):
#                 Nid = i
#                 bit_dict[Nid]= 1-side 
#                 locked_nodes[Nid]=True
#             else:    
#                 break
                
#         elif gain==0:
#             num_zero_p+=1
#             if gain_pos==maxDegree:
#                 print( 'nodes: '+str(i))
#                 print(gain_pos)
#                 if not locked_nodes[i] and move_nid_balance_redundancy_check(bit_dict, side, i,alpha, cur_rate,ideal_part_in_size):
#                     print('come in ')
...
#     #     gain=gain_pos-gain_neg 
#     #     if gain>0:
#     #         print('B_o gain is positive '+str(gain))
#     #         if not locked_nodes[i] and move_nid_balance_redundancy_check(bit_dict, side, i,alpha, cur_rate,ideal_part_in_size):
#     #             maxGainNid = i
#     #             ifFound = True
#     #             # cur_bucket[maxLeftGainIndex-index].remove(i)
...
#     #         print('\t : ', num_zero_n/idx)    
#     #     # gains=initializeBucketSort(i,gain, 0) # 0: left side bucket
#     # print('rate of zero positive: ', num_zero_n/idx)
#     # if maxGainNid == None: ########
#     #     side = 1-side 
#     #     return   
...
def updateRed_group(locked_nodes, cur_rate, alpha,ideal_part_in_size):
    ...
    #     gain=gain_pos-gain_neg 
    #     if gain>=0:
    #         nid_to_move.append(i)
        
    #     if idx>0 and idx%100==0:
    #         print('\t \t\tA_o : ', len(nid_to_move)/idx)   
    
    print('the number of node to move A-> B is :', len(nid_to_move))
    print()
    if len(nid_to_move)>0:
    ...
    #     if gain>=0:
    #         if not locked_nodes[i] :
    #             ready_to_move.append(i)
                
    #     if idx>0 and idx%100==0:
    #         print('\t : ', len(ready_to_move)/idx)    
        # gains=initializeBucketSort(i,gain, 0) # 0: left side bucket
...
def walk_terminate_0( block_to_graph,rate,args, ideal_part_in_size):
    ...
    bestRate = rate   # best rate in this segment
    ...
    A_o, B_o=initializeGain() # compute initial gain for the segment
    t3=time.time()
    ...
    for i in range(steps_):
        tt=time.time()
        updateGain(locked_nodes,rate,args.alpha,ideal_part_in_size)
        tt_e=time.time()
        if i % 1==0:
...
def walk_terminate_1(g,red_rate, args,ideal_part_in_size):
    ...
    for i in range(steps_):
        ...
        tt_e=time.time()
        if i % 1==0:
            print('\t\t\tone update redundancy spend \t', tt_e-tt)	
            print('\t\t  ---------------------------------------------------- n',i)
        totalSteps+=1
        flag, len_A_part,len_B_part = balance_checking(args.alpha)
        print('\t\t\t-------------------------------------len_A_part: ', len_A_part)
        print('\t\t\t-------------------------------------len_B_part: ', len_B_part)
        tmpRate,ratio_A,ratio_B =getRedundancyRate(len_A_part,len_B_part,ideal_part_in_size)
        print('\t\t\tredundancy rate: '+str(tmpRate)+' '+str(ratio_A)+' '+ str(ratio_B))
        if flag:
            tmpRate,ratio_A,ratio_B =getRedundancyRate(len_A_part,len_B_part,ideal_part_in_size)
            # cutcost= getCost(A_o,B_o,bit_dict)
            # print('\t\t\t-------------------------------------cutcost: ', tmpRate)
...
def graph_partition_variant( batched_seeds_list, block_2_graph, args):
    ...
    for i in range(num_batch-1):# no (end, head) pair
        ...
        tij=time.time()
        print('\tpreparing two sides time : ' , time.time()-tii)
        ...
        while pass_<3:
            if args.walkterm==0:
                ti=time.time()
                print('before terminate 0 the redundancy rate: ', red_rate)
                improvement,red_rate=walk_terminate_0(g,red_rate, args,ideal_part_in_size)
                print('\twalk terminate 0 spend ', time.time()-ti)
                print('\tafter improvement ',improvement)
                print('\tafter terminate 0 the  redundancy rate ',red_rate)
                if not improvement:
                    # print(cost)
...
def graph_partition_quick( batched_seeds_list, block_2_graph, args):
    ...
    for i in range(num_batch-1):# no (end, head) pair
        ...
        tik=time.time()
        red_rate,ratio_A,ratio_B =getRedundancyRate(len_A_part,len_B_part,ideal_part_in_size) #r_cost=max(r_A, r_B)
        print('\tget Redundancy rate: ' , time.time()-tik)
        if red_rate < 1.0:
            continue
        # cutcost = getCost(A_o,B_o,bit_dict)
        # print('\tgetRedundancyCost: ' , time.time()-tik)
