...
def getRedundancyRate(len_A, len_B, ideal_part_in_size):
    ...
    return rate   #   minimize the max ratio of 
...
def get_two_partition_seeds():
    ...
    # pool = mp.Pool(mp.cpu_count())
    # # results = [pool.apply(calculate_gain_A_o, args=(i, A_o, B_o)) for i in A_o]
    # A_o = pool.starmap_async(get_p_nid, [ (i,0) for  idx,i in enumerate(bit_dict)]).get()
    # pool.close()
    # print(A_o)
    # A_o=list(filter(exists, A_o))
    # print(A_o)
    
    # pool = mp.Pool(mp.cpu_count())
    # B_o = pool.starmap_async(get_p_nid, [ (i,1) for  idx,i in enumerate(bit_dict)]).get()
    # pool.close()
    
    # B_o=list(filter(exists, B_o))
    # print(B_o)
...
def move_nid_balance_redundancy_check(bit_dict_origin, side, nid,alpha, red_rate,ideal_part_in_size):
    ...
    red=getRedundancyRate(len_A_part, len_B_part, ideal_part_in_size)
...
def walk_terminate_0( block_to_graph,rate,args, ideal_part_in_size):
    ...
    for i in range(steps_):
        ...
        if flag:
            tmpRate=getRedundancyRate(len_A_part,len_B_part,ideal_part_in_size)
...
def graph_partition( batched_seeds_list, block_2_graph, args):
    ...
    for i in range(num_batch-1):# no (end, head) pair
        ...
        tik=time.time()
        red_rate=getRedundancyRate(len_A_part,len_B_part,ideal_part_in_size) #r_cost=max(r_A, r_B)
