...
def gen_src_nodes_dict(raw_graph):
    global partitions
    global block_to_graph
    partitions={}
    ...
    for nid in sub_out_nids:
        ...
        # local_in_nids = list(block_to_graph.in_edges(local_nid))[0].tolist()
    ...
    #     temp=list(set(in_nids).intersection(sub_in_nids))
    #     partitions[nid]=temp
    #     # print('i------ ',iii)
    #     iii+=1
    #     if iii%2000==0:
    #         print(time.time()-tt)
        ...
        #     # print('j---------- ',jjj)
        #     # jjj+=1
        #     # block_to_graph.in_edges
        #     # t100=time.time()
        #     in_nids = list(raw_graph.in_edges(nid))[0].tolist()
        #     # print('in_nids generation time: ', time.time()-t100)
        #     # temp=[]
        #     # for in_i in in_nids:
...
def graph_partition(raw_graph, batched_seeds_list, block_2_graph, args):
    ...
    print('{}-'*40)
    t_b=time.time()
    partitions=gen_src_nodes_dict(raw_graph)
    print('gen_src_nodes_dict partition generation spend ',time.time()-t_b)
